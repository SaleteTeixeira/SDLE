...
public class Terminal implements Runnable {
        ...
        do {
            ...
            switch (op) {
                ...
                case 2:
                    ...
                    List<Client> pubs = this.node.listPublishersClients();
                    for (Client c : pubs) {
            ...
            switch (op) {
                case 1:
                    ...
                    if (i != 1) {
                        if (scan.hasNextInt()) {
                            try {
                                int v = scan.nextInt();
                                this.node.addPublisher(aux.get(v - 1).getKey());
                ...
                case 2:
                    ...
                    if (scan.hasNextLine()) {
                        ...
                        if(this.node.getNetwork().containsKey(key)){
                            if (this.node.getPublishers().contains(key)) {
                            ...
                            System.out.println("Error: Invalid key. That key doesn't belong to anyone in the network.");
                        }
                    }
                ...
                case 3:
                    for (Map.Entry<String, List<String>> e : this.node.getSuggestedPubsByPub().entrySet()) {
                        if (e.getValue().size() != 0) {
                            System.out.println("Sugested by " + this.node.getNetwork().get(e.getKey()).getUsername() + ": " + e.getKey() + "\n");
                            i = 1;
                            ...
                            System.out.println("Publisher " + this.node.getNetwork().get(e.getKey()).getUsername() + ": " + e.getKey() + " hasn't suggested anyone yet.");
                        }
                    }
    ...
    private void unsubscribe() {
        ...
        List<Client> aux = this.node.listPublishersClients();
        for (Client c : aux) {
            System.out.println(i + ". " + c.getUsername() + ": " + c.getKey());
            i++;
        }
